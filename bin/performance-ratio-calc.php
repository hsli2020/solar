...
$Temperature_Losses = $Maximum_Theory_Output * $Module_Power_Coefficient * 
                      (25 - $Avg_Module_Temp) / 1000.0;
$Inverter_Losses = (1 - $Inverter_Efficiency) * ($Maximum_Theory_Output - $Temperature_Losses);

$x = $Maximum_Theory_Output - $Temperature_Losses - $Inverter_Losses;

if ($x > $AC_Nameplate_Capacity)
    $Inverter_Clipping_Loss = $x - $AC_Nameplate_Capacity;
else
    $Inverter_Clipping_Loss = 0;

$Transformer_Losses  = ($x - $Inverter_Clipping_Loss) * $Transformer_Loss;
$Other_System_Losses = ($x - $Inverter_Clipping_Loss - $Transformer_Loss) * $Other_Loss;
$Total_Losses = ($Temperature_Losses + $Inverter_Losses + $Inverter_Clipping_Loss + 
    $Transformer_Loss + $Other_System_Losses) / $Maximum_Theory_Output;
$Theoretical_Output = ($x - $Inverter_Clipping_Loss - $Transformer_Loss - $Other_System_Losses);
